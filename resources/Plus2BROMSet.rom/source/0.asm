; Spectrum +2B Monster ROM - editor

; Based on the UK 128 ROM 0 disassembly by Matthew Wilson (2011-12-13)
; Bug-fixed by Paul Farrow

BUG_FIXES equ 1 ; Specify to patch with fixes for all known bugs within ROM 0
...
;        DEFW TEST_SCREEN  ; $3C04. Will return if BREAK is not being pressed.
		...
		nop					; fix to work with original BASIC 
		nop
		nop
...
if BUG_FIXES = 0
        ...
        XOR  A            ; Set the zero flag and clear the carry flag.
        LD   A,$01
        JP   L0865
endif
...
; BUG FIXES
; =========
; Bug fixes produced by Paul Farrow.

if BUG_FIXES = 1

; Overwrite the unused routines at $2336 with bug fixes.

        ORG $2336

; -------------------------
; Colour Control Parameters
; -------------------------
...
BUG_FIX_3:
        ...
        XOR   A           ; Zero flag set and carry flag reset.
        RET               ;
...
BUG_FIX_6:
        LD   (IY+$31),$02 ;
        JP   L30D6        ; Reset Below-Screen Line Edit Buffer settings to their default values.

; ------------------------------------
; Delete a Character from a BASIC line
; ------------------------------------
...
BUG_FIX_7:
        ...
        JR   Z,BF7_EXIT   ; Jump ahead if not.
        
        LD   B,$00        ;
        CALL L2E41        ; Find first editable position on this row searching to the right, returning column number in B.
        JR   C,BF7_EXIT   ; Carry set if character exists on the line.

        LD   HL,$EC0D     ;
        RES  3,(HL)       ; Current line has not been altered.
...
BUG_FIX_8:
        ...
        JP   L2A11        ; Store cursor editing position.

BUG_FIX_9:
        LD   A,($F6F0)    ;
        JP   L29F8        ; Store editing position and print cursor.
...
BUG_FIX_10:
        ...
        DEC  HL           ;
        DEC  HL           ;
        JP   L32D6        ; Copy a BASIC line into the Above-Screen Line Edit Buffer.
...
BUG_FIX_12:
        RST  28H          ;
        DEFW FP_TO_BC     ; $2da2. Fetch the number line to BC.
        JP   C,BUG_FIX_12_RET2 ;
...
BUG_FIX_14:
        ...
        LD   ($FD89),A    ; Reset the '<' and '>' store.
        JP   BUG_FIX_14_RET ;

BUG_FIX_15:
        LD   A,($FD89)    ;
        AND  A            ; Was the last character a '>' or '<'?
        JR   Z,BF15_INSERT ; Jump if not.

        PUSH BC           ; Save the new chracter.
        LD   B,A          ;
        CALL L3E64        ; Insert the '>' or '<' into the BASIC line workspace.
        POP  BC           ; Retrieve the new character.
        XOR  A            ;
        LD   ($FD89),A    ; Clear the '>' or '<'.

BF15_INSERT:
        JP   L3DE9        ; Insert the character into the Keyword Conversion Buffer.
...
BUG_FIX_16:
        ...
        JR   NZ,BF16_WITHIN ; Jump forward if within quotes.

;Not within quotes

        BIT  2,A          ; Within a REM statement?
        JR   Z,BF16_NOT_WITHIN ; Jump forward if not within a REM statement.
...
;Within a REM statement
	...
	PUSH AF           ;
        CP   ':'          ;
        JR   NZ,BF16_WITHIN ; Jump if not a colon.

        LD   A,($FD81)    ;
        AND  $FB          ; Signal not within a REM statement.
        LD   ($FD81),A    ;
        
;Not within a REM statement or quotes

BF16_WITHIN:
        POP  AF           ; Retrieve the character to insert.
        JP   L3E64        ; Simply insert the character into the BASIC line workspace.
...
BUG_FIX_17:
        ...
        SBC  HL,DE        ; Substract the number of characters in the keyword text.
        LD   B,H          ;
        LD   C,L          ; Transfer the result to BC, and then return via the instructions at $3F2A onwards.
...
BUG_FIX_18:
        ...
        DEC  HL           ; Move back to the current character.
        EX   DE,HL        ; DE=Next insert position within the tokenized BASIC line.
        CP   $0E          ; Is it the 'number' marker?
        JP   NZ,L3F5D     ; Jump ahead if not.

        INC  DE           ; Skip over additonal byte of embedded floating point number.
        JP BUG_FIX_18_RET ;

; -------------------------
; Insert/Execute BASIC Line
; -------------------------
...
BUG_FIX_19:
        ...
        POP  AF           ; Restore the flags.
        JR   C,BF23_FOUND ; Jump if editable column found.
 
        LD   A,(DE)       ; Fetch the flag byte.
        BIT  1,A          ; Does the BASIC line span onto the next row?
        JR   NZ,BF_SPANS  ; Jump if it does.
 
        POP  DE           ; DE=Address of the start of the BASIC row in the Screen Line Edit Buffer.
        POP  HL           ;
        LD   HL,$0000     ; Signal no editable characters left on the row.
        LD   ($EC06),HL   ;
        JP   L15C0        ; Jump since all characters on the row have been scanned through.
 
BF_SPANS:
        POP  DE           ; DE=Address of the start of the BASIC row in the Screen Line Edit Buffer.
        POP  HL           ;
        JP   L15CB        ; Jump if no editable columns left on the row.
...
; -------------------------------------------------------
; Find Next Screen Edit Buffer Editable Position to Right
; -------------------------------------------------------

BUG_FIX_21:
        LD   HL,-35       ;
        ADD  HL,DE        ;
        EX   DE,HL        ; DE=DE-35 to point to previous row
        JP   L35F4        ; Indent the row by setting the appropriate number of null characters in the current Screen Edit Buffer row.
...
BUG_FIX_22:
        ...
        RET  C            ; Return if not to fetch and return the first character of the keyword string.

        CP   $25          ; Was it '<>' or below?
        RET  NC           ; Return if not to fetch and return the first character of the keyword string.

        LD   HL,($FCA1)   ; Is there a leading space?
        LD   A,(HL)       ;
        CP   ' '          ; $20.
        JR   NZ,BF22_NOT_LEADING ; Jump if there is not.
...
BF23_START:
        ...
        LD   HL,$2000     ; Number of samples to monitor for changes in input line state.
        
BF23_LOOP:
        IN   A,(C)        ; Read the keyboard and tape input port.
        BIT  0,A          ; Test for SPACE (i.e. BREAK).
        JP   Z,L3C56      ; Exit Tape Tester if SPACE/BREAK pressed.
...
BUG_FIX_25:
        ...
        CALL NZ,L3EFB     ; If so then insert the character into the BASIC line workspace.
...
; CREDITS (IN FULL)
	...
	defb "EDITOR credits: Martin Brennan, Steve Berry, Andrew Cummins, Rupert Goodwins, & Kevin Males"
...
L2604:
if BUG_FIXES = 0
        CALL L30D6        ; Reset Below-Screen Line Edit Buffer settings to their default values.
else
        CALL BUG_FIX_6
endif
...
L27CB:  DEFB $04          ; Number of entries.
        DEFB $00
        DEFW L286C        ; 128 BASIC option handler.
        DEFB $01
...
L27D2:  DEFB 05           ; Number of entries.
        DEFM "Options "
        DEFB $FF
        DEFM "128 BASI"
        DEFB 'C'+$80
...
; ------------------------------------
; Calculator Menu - Tape Tester Option
; ------------------------------------
...
L2816:  CALL L3857        ; Clear screen and print the "Tape Tester" in the banner.
		...
		JP   L2885			; return to the Calculator

; ------------------------------------
; Calculator Menu - Tape Loader Option
; ------------------------------------
...
L30D0:
if BUG_FIXES = 0
        DEFB $05          ; Number of bytes in table.
        ...
        DEFB $03          ; Number of bytes in table.
        DEFB $00          ; $F6F5 = Number of rows held in the Below-Screen Line Edit Buffer.
...
L3683:  BIT  5,(HL)       ;
        ...
        CP   $03          ; Is it a cursor key or EDIT? (or TRUE VID / INV VID) [modified]
...
if BUG_FIXES = 0
        ...
        LD   DE,$07C0     ; Read 1984 tape samples.
endif

...
fn select_hw_model(version: Z80Version, head_ex: &HeaderEx) -> Option<ComputerModel> {
...
fn mem_page_to_range(page: u8, model: ComputerModel) -> Option<MemoryRange> {
    use ComputerModel::*;
    match model {
        Spectrum16(ext)|Spectrum48(ext)|
        Tc2048(ext)|Ts2068(ext)|Tc2068(ext) => {
            Some(match page {
        ...
        Spectrum128(ext)|SpectrumPlus2(ext)|
        SpectrumPlus2A(ext)|SpectrumPlus3(ext)|SpectrumPlus3e(ext)|
        SpectrumSE(ext) => {
            Some(match page {
...
{
    let header = load_header(rd.by_ref())?;
    let mut version = Z80Version::V1;

    let mut model = ComputerModel::Spectrum48(Default::default());
    ...
    let header_ex = if cpu.get_pc() == 0 {
        ...
        model = select_hw_model(version, &head_ex).ok_or_else(||
    ...
    let issue = if flags2.is_issue2_emulation() {
        ReadEarMode::Issue2
    }
    else {
        ReadEarMode::Issue3
    ...
    // TODO better issue handling (Clear on +2A and above)

    loader.select_model(model, border, issue, Some(joystick))
    ...
    if version == Z80Version::V1 {
        if flags1.is_mem_compressed() {
            ...
            let buf = match buf.get(buf.len() - 4..) {
                Some(&[0, 0xED, 0xED, 0]) => &buf[..buf.len() - 4],
                _ => &buf[..]
            };
            let decompress = MemDecompress::new(buf);
            loader.load_memory_page(MemoryRange::Ram(0..3*PAGE_SIZE), decompress)?;
        }
        else {
            loader.load_memory_page(MemoryRange::Ram(0..3*PAGE_SIZE), rd)?;
        }
    }
    else {
        while let Some((len, page, is_compressed)) = load_mem_header(rd.by_ref())? {
            let range = mem_page_to_range(page, model).ok_or_else(||
            ...
            if is_compressed {
                ...
                let decompress = MemDecompress::new(&buf);
                loader.load_memory_page(range, decompress)?;
            }
            else {
                loader.load_memory_page(range, rd.by_ref().take(len as u64))?;
            }
        }
    }
    ...
    if let Some(head_ex) = header_ex {
        ...
        match model {
            ...
            SpectrumPlus2A(ext)|SpectrumPlus3(ext)|SpectrumPlus3e(ext)|
            SpectrumSE(ext)
            if head_ex.ifrom == 0xff && ext.intersects(Extensions::IF1) => {

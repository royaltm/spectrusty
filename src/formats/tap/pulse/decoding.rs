...
impl<W: Write> PulseDecodeWriter<W> {
    ...
    /// transfer such as an iterator returned from [crate::audio::MicOut::mic_out_pulse_iter].
    /// However providing an empty iterator is equivalent to calling [PulseDecodeWriter::end] thus
    /// ending the current transfer in progress if there is any.
    ///
